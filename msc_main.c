  ...
  */
  ...
  if(ret == LPC_OK) {
    ...
    if((pIntfDesc == 0) || (pIntfDesc->bInterfaceClass != USB_DEVICE_CLASS_STORAGE) || (pIntfDesc->bInterfaceSubClass != MSC_SUBCLASS_SCSI)) {
      ...
      putLineUART(buffer);
      sprintf(buffer,"%d, %d, %d",pIntfDesc, pIntfDesc->bInterfaceClass, pIntfDesc->bInterfaceSubClass);
      putLineUART(buffer);
      errorUSB();
    }
    ...
    if(ret == LPC_OK) {
      ...
      sprintf(buffer,"USB ERROR: inner, ret = %d", ret);
      putLineUART(buffer);
      errorUSB();
    }
  }
  else {
	sprintf(buffer,"USB ERROR: outer, ret = %d", ret);
	putLineUART(buffer);
    errorUSB();
  }
